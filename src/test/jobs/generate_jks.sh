...
openssl s_client -connect "$1:$2" </dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > tmp_cert_registry
...
keytool -import -noprompt -trustcacerts -keystore "$JKS_FILE" -storepass "$PASSWORD" -alias "$1" -file tmp_cert_registry
